...
//		}
	...
	protected void onDestroy() {
		...
		clue.SendLog("Application end", "Application entered onDestroy()");
		super.onDestroy();
	}
		...
		for( int i = 0; i < games.length; i++ ) {
			...
			views[i].setOnClickListener(new View.OnClickListener() {
				public void onClick(View arg1) {
					...
					i.setData(Uri.fromParts("Moriarty", gameId, null));
					Clues clues = new Clues();
					clues.SendLog("Game start", "User chosen to play a game (name);" + gameId);
					getGame = gameId;
					startActivity(i);
