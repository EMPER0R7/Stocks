...
import com.bgu.sherlock.Moriarty.compat.PrefsSaver;
import com.bgu.sherlock.Moriarty.compat.SysUIVisSetter;
...
public class GamePlay extends AppCompatActivity implements OnSharedPreferenceChangeListener, NightModeHelper.Parent {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(GameChooser.getGame != null)
        clues.SendLog("Game stared", "User started to play a game (name);"+ GameChooser.getGame);
        else clues.SendLog("Game stared", "User started to play a game (name);"+state.getString(SAVED_BACKEND, null));
...
//        final Intent beamIntent = new Intent(this, getClass());
//        beamIntent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
//
//        PendingIntent pendingIntent = PendingIntent.getActivity(
//                this, 0, beamIntent, PendingIntent.FLAG_UPDATE_CURRENT);
//
//        NdefMessage ndefMessage = createNdefMessage(); // Implement your NdefMessage creation logic
//
//        nfcAdapter.setNdefPushMessage(ndefMessage, this, this);
//
//        // If you want to set a callback when the Beam is complete, you can use setOnNdefPushCompleteCallback
...
//        // Implement your NdefMessage creation logic
//        String saved = saveToString();
//        if (saved == null) return null;
//
//        NdefRecord mimeRecord = createMime(saved);
...
//    }
    ...
    private void startGameThread(final GameLaunch launch) {
        workerRunning = true;
        (worker = new Thread(launch.needsGenerating() ? "generateAndLoadGame" : "loadGame") {
            public void run() {
                try {
    ...
    protected void onPause() {
        clue.SendLog("App Mode change", "App entered onPause()");
    ...
    protected void onResume() {
        clue.SendLog("App Mode change", "App entered onResume");
        super.onResume();
        if (restartOnResume) {
    ...
    protected void onDestroy() {
        Clues clues = new Clues();
        clues.SendLog("Game end", "User exited from game (name);" + state.getString(SAVED_BACKEND, null));
        stopNative();
        super.onDestroy();
    }
