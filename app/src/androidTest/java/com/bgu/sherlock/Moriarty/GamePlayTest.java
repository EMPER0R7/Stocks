...
public class GamePlayTest {
	...
	@Rule
	public ActivityScenarioRule<GamePlay> mActivityRule =
			new ActivityScenarioRule<>(GamePlay.class);
	...
	public static Iterable<Object[]> data() {
		Set<String> unusedBackends = new LinkedHashSet<>(Arrays.asList(
				ApplicationProvider.getApplicationContext().getResources().getStringArray(R.array.games)));
		addExamples();
	...
	public void testGameCompletion() throws InterruptedException {
		...
		final Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("Moriarty:" + _backend + ":" + _gameID));
		ActivityScenario<GamePlay> scenario = ActivityScenario.launch(intent);
		final GamePlay[] activity = {null};

// Access the activity instance within the scenario
		scenario.onActivity(new ActivityScenario.ActivityAction<GamePlay>() {
			@Override
			public void perform(GamePlay mActivity) {
				activity[0] = mActivity;
			}
		});
	...
	private void assertCompleted(final boolean isCompleted) {
		...
		final Matcher<View> titleMatcher = withText(containsString(ApplicationProvider.getApplicationContext().getString(R.string.COMPLETED)));
		if (isCompleted) {
