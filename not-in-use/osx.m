...
- (id)initWithGame:(const game *)g
{
    ...
    self = [super initWithContentRect:rect
	    ...
	    backing:NSBackingStoreBuffered
	    defer:YES];
    [self setTitle:[NSString stringWithUTF8String:ourgame->com]];
...
- (void)saveGame:(id)sender
{
    ...
    if ([sp runModal] == NSFileHandlingPanelOKButton) {
       const char *com = [[sp filename] UTF8String];
...
- (void)loadSavedGame:(id)sender
{
    ...
    if ([op runModalForTypes:nil] == NSOKButton) {
         ...
         * moment.
         */
	const char *com = [[[op filenames] objectAtIndex:0]
...
- (void)becomeKeyWindow
{
	...
	while (n--) {
	    ...
	    midend_fetch_preset(me, n, &com, &params);

	    item = [[[DataMenuItem alloc]
		     initWithTitle:[NSString stringWithUTF8String:com]
...
- (void)startConfigureSheet:(int)which
{
	...
	switch (i->type) {
	  case C_STRING:
	    ...
	    [tf setBordered:NO];
	    [tf setDrawsBackground:NO];
	    [[tf cell] setTitle:[NSString stringWithUTF8String:i->com]];
	    [tf sizeToFit];
	    rect = [tf frame];
	  ...
	  case C_BOOLEAN:
	    ...
	    [b setTitle:[NSString stringWithUTF8String:i->com]];
	    [b sizeToFit];
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < gamecount; i++) {
	    id item =
		initnewitem([DataMenuItem allocWithZone:[NSMenu menuZone]],
			    submenu, gamelist[i]->com, "", controller,
			    @selector(newGameWindow:));
	    [item setPayload:(void *)gamelist[i]];
	}
    }
